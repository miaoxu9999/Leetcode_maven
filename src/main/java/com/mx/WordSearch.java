...
public class WordSearch {
    ...
    public boolean exist(char[][] board, String word) {
        ...
        boolean[][] flaglist = new boolean[board.length][board[0].length];
    ...
    public void dfs(char[][] board, String word, int step, int x, int y, boolean[][] flaglist)
    {
        if (step == word.length())
        {
            flag = true;
        ...
        if (x < 0 || y < 0 || x >= board.length || y >= board[0].length || board[x][y] != word.charAt(step) || flaglist[x][y])
        {
            return;
        }
        else
        {
            flaglist[x][y] = true;
            //判断上下左右有没有step位的字母
            dfs(board, word, step + 1, x -1, y, flaglist);
            if (flag)
            {
                return;
            }
            dfs(board, word, step + 1, x , y -1, flaglist);
            if (flag)
            {
                return;
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println( new WordSearch().exist(b, w));
    }
}
